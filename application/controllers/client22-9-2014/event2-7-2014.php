...
//            redirect(SITE_URL.'events');
    ...
    function send_message()
    {
        ...
        $json_array['error']                                                    = 'error';
        $json_array['msg']                                                      = 'Something Went Wrong';
        $get_organizer                                                          = $this->model->getOrganizer($event_id);
            ...
            else
            {
                ...
                if($mulitple_attendee_id)
                {
                    ...
                    foreach($mulitple_attendee_id as $k => $v)
                    {
                        ...
                        $subject                                                = 'Procialize: Personal message sent from '.$this->session->userdata('client_first_name');
                        ...
                        $html                                                   = str_replace('{object_first_name}',  $this->session->userdata('client_first_name'),$html);
        ...
        if($data)
        {
            foreach($data as $key => $attendee)
            {
                $html                                                          .= '<div class="col-xs-12"><a href="'.SITE_URL.EVENT_CONTROLLER_PATH.'attendee-detail/'.$attendee['attendee_id'].'"><div class="stat well well-sm attnd"><div class="row">
                                                                                   ...
                                                                                   </div></div><div class="col-xs-6 eventdet"><h4>'.$attendee['attendee_name'].'</h4><small class="stat-label">'.$attendee['attendee_designation'].' - '.$attendee['attendee_company'].'</small>
                                                                                   <small class="stat-label">'.$attendee['attendee_industry'].', '.$attendee['attendee_functionality'].'</small></div><div class="col-xs-2"><small class="stat-label attp">'.$attendee['attendee_city'].'</small></div>
    ...
    function event_registration()
    {
        ...
        if($event_id && $user_id)
        {
            ...
            {
                $json_array['error']                                            = 'success';
                $json_array['msg']                                              = 'You have already registered for this Event, Organizer will send you the Passcode to enter the Event.';
            }
            ...
            if(!$check_attendee &&  $get_organizer)
            {
                ...
                sendMail($to, $subject, '',$html);
                $json_array['error']                                            = 'success';
                $json_array['msg']                                              = 'You have registered successfully for this event!';
                //send the mail
            }
        }
        echo json_encode($json_array);
...
//        /echo 'test';
    ...
    function set_meeting()
    {
        ...
        $json_array['error']                                                    = 'error';
        $json_array['msg']                                                      = 'Something went Wrong!';
        $get_organizer                                                      = $this->model->getOrganizer($data['event_id']);
        if($data['start_time'] && $data['end_time'] && $data['target_id'] && $data['title'])
        {
            ...
            if($set_meeting)
            {
                ...
                $subject                                                        = 'Procialize: Meeting request sent from '.$this->session->userdata('client_first_name');
    ...
    function reply_meeting()
    {
        ...
        if($data['target_id'] && $data['meeting_id'] && $data['attendee_id'] && $data['responce'])
        {
            ...
            if($reply_meeting)
            {
                ...
                $get_target_attendee                                            = $this->client_login_model->getUserData($data['target_id']);
                $subject                                                        = 'Procialize: Response to meeting request from '.$this->session->userdata('client_first_name');
