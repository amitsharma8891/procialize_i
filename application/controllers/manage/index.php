...
//        echo $this->db->last_query();exit;
        ...
        $event_list = $this->dashboard_model->getDistinctEventView($this->event_id);
        $event_visit = count($event_list);
        $facebook = 0;
        $linkden = 0;
        $manual = 0;
...
//        echo '<pre>';print_r($arrSession);exit;
        ...
        $arrAppAttendee = $this->report_model->get_user('app_used_by_user', $this->event_id);
        $user_signed_into_app = $this->report_model->get_user_signed_into_app('get_user_signed_into_app', $this->event_id);

        $android_count = 0;
        $ios_count = 0;
        foreach ($user_signed_into_app as $value) {
            if ($value['mobile_os'] == 'android')
                $android_count++;
            if ($value['mobile_os'] == 'ios')
                $ios_count++;
        }
        //show_query();
        //display($arrAppAttendee);
        //echo count($arrAppAttendee);
        /* $arrAppAttendee = 0;
          foreach ($arrAppAttendee as $eachAttendee) {
          //if ($eachAttendee['status'] == 1)
          $arrAppAttendee++;
          } */
        $registerAttendee = count($arrAttendee);

        $arrData['android_count'] = $android_count;
        $arrData['ios_count'] = $ios_count;
        $arrData['event_visit'] = $event_visit;
        $arrData['attendeeApp'] = count($arrAppAttendee);
