    ...
    function total_audience($report_for, $organizer_id, $event_id) {
        ...
        switch ($report_for) {
            ...
            case 'exhibitor':
                 ...
                 $get_attendee = $this->exhibitor_model->getAll($event_id, false, null, array(), 'LIKE', 'event_has_attendee.event_id');
                //display($get_attendee);
                $excel_column = "User Type\t Name\t Designation\t Company Name\t Email ID\t Mobile No\t Contact No\t Visited Event?\t Accessed App?\n";
                if ($get_attendee) {
                    foreach ($get_attendee as $attendee) {
                        //if($attendee['attendee_type'] == 'A')
                        $sender_type = $this->get_user_type($attendee['attendee_type']);
