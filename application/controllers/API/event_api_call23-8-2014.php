...
class Event_api_call extends CI_Controller {
    ...
    function send_message()
    {
        ...
        if($event_id && $attendee_id && $message && $get_organizer )
        {
            if($checkbox)
            {
                ...
                $send_msg                                                       = $this->model->send_mesage($attendee_id,$attendee_type,md5($message_id->message_count),0,$target_user_type,$message,$event_id);
            }
            else
            {
                if($mulitple_attendee_id)
                {
                    ...
                    foreach($mulitple_attendee_id as $k => $v)
                    {
                        ...
                        $send_msg                                               = $this->model->send_mesage($attendee_id,$attendee_type,md5($message_id->message_count),$v,$target_user_type,$message,$event_id);
                        
                        $get_target_attendee                                    = $this->login_model->getUserData($target_attendee_id);
                        //$subject                                                = 'Procialize: Personal message sent from '.$this->session->userdata('client_first_name');$get_organizer->event_name.' - Networking App by Procialize -
						$subject                                                = $get_organizer->event_name.' - Networking App by Procialize - '.$user_data->first_name.' has sent a message ';
                ...
                else
                {
                    ...
                    $send_msg                                                   = $this->model->send_mesage($attendee_id,$attendee_type,md5($message_id->message_count),$target_attendee_id,$target_user_type,$message,$event_id);
    ...
    function reply_message()
    {
        ...
        $message                                                                = mysql_real_escape_string($this->input->post('message_text',TRUE));
        $target_attendee_id                                                     = $this->input->post('target_attendee_id');
        $target_attendee_type                                                   = $this->input->post('target_attendee_type');
    ...
    function session_rsvp()
    {
        ...
        if($session_id && is_numeric($session_id) && $get_organizer)
        {
            ...
            $check_rsvp                                                         = $this->model->check_rsvp($session_id,$attendee_id);
            if($check_rsvp)
            {
                $json_array['error']                                            = 'error';
                $json_array['msg']                                              = 'You have already RSVPed for this session!';
                json_output($json_array);
                exit;
            }
			...
			$subject                                                            = $get_organizer->event_name.' - Networking App by Procialize - '.$user_data->first_name.' has registered for Session '.@$get_session[0]['session_name'];
            $html                                                               = str_replace('{event_name}',$get_organizer->event_name,mail_to_organizer_when_attendee_rsvp());
            $html                                                               = str_replace('{session_name}',@$get_session[0]['session_name'],$html);
    ...
    function event_registration()
    {
        ...
        $event_id                                                               = $this->input->post('event_id');
        $json_array['error']                                                    = 'error';
        $json_array['msg']                                                      = 'Invalid Event ID or Access Token';
        if(is_numeric($event_id) && $api_access_token)
        {
            $check_attendee                                                     = $this->model->checkAttendee($event_id,$api_access_token);
            $user_data                                                          = $this->model->getUser($api_access_token);
            $get_organizer                                                      = $this->model->getOrganizer(NULL,$event_id);
            ...
            if(!$check_attendee && $user_data)
            {
                ...
                $subject                                                        =   'Procialize: Registration request for Event';
				$subject                                                        = $get_organizer->event_name.' - Networking App by Procialize - Event registration request by '.$user_data->first_name ;
    ...
    function set_meeting()
    {
        ...
        if($data['start_time'] && $data['end_time'] && $data['target_id'] && $data['title'] && is_numeric($data['event_id']))
        {
            ...
            if($set_meeting)
            {
                ...
                $get_target_attendee                                            = $this->login_model->getUserData($data['target_id']);
                $subject                                                        = 'Procialize: Meeting request sent from '.$user_data->first_name;
				$subject                                                        = $get_organizer->event_name.' - Networking App by Procialize - '.$user_data->first_name.' has requested for a meeting';
    ...
    function reply_meeting()
    {
        ...
        if($data['target_id'] && $data['meeting_id'] && $data['attendee_id'] && $data['responce'])
        {
            ...
            if($reply_meeting)
            {
                ...
                $get_target_attendee                                            = $this->login_model->getUserData($data['target_id']);
                $subject                                                        = 'Procialize: Response to meeting request from '.$user_data->first_name;
				...
				if($get_target_attendee->gcm_reg_id)
				{
					...
					$mobile_message = $user_data->first_name.' '.bracket_attendee_attribute($user_data->designation,$user_data->company_name,'-').' replied to your meeting request.';
					$responce = $this->mobile_model->send_notification($get_target_attendee->gcm_reg_id,$get_target_attendee->mobile_os,$mobile_message);
					//echo $responce;
				}
    ...
    function linkedin_login()
    {
        ...
        {
            //display($data);exit;
            //display($data['skills']);//exit;
            //display($data['location']);//exit;
            //$data['company']                                                    = $data['company'];//@$data['position']['position']['company']['name'];
            //$data['designation']                                                = @$data['position']['position']['title'];
