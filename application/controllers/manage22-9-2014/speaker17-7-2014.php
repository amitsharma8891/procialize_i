...
class Speaker extends CI_Controller {
    ...
    public function index($order = NULL) {
        ...
        if ($order == 3) {
            ...
            $arrData['list'] = $this->speaker_model->getAll(NULL, FALSE, $search, $field, 'AND');
        } elseif ($order == 4) {
            $search = '1';
            $field = array("attendee.mail_sent");
            $arrData['list'] = $this->speaker_model->getAll(NULL, FALSE, $search, $field, 'AND');
        } else {
            $arrData['list'] = $this->speaker_model->getAll();
        }
        if ($this->input->post()) {
            if ($this->input->post('send_mail') != '') {
                $status = $this->speaker_model->sendPasscode($this->input->post('delete'));
                if ($status) {
