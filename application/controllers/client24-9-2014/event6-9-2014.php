...
class Event extends CI_Controller {
    ...
    function reply_message() {
        ...
        if ($attendee_id && $message && $target_attendee_id && $message_id) {
            ...
            $this->common_transaction_model->message = $message;
            $this->common_transaction_model->event_id = $event_id;
            $send_msg = $this->common_transaction_model->common_send_message($type = 'reply', json_decode($user_data), $event_id, $target_attendee_id, $target_user_type);
...
//                $save_attendee = $this->model->insert_attendee($attendee_id, $event_id);
        ...
        $to = $get_organizer->email; //$this->session->userdata('client_email');
        $email_template = get_email_template('mail to organizer for registration request');
//        $subject = 'Procialize: Registration request for session';
        $keywords = array('{app_name}', '{event_name}', '{org_name}', '{first_name}', '{email}', '{app_contact_email}', '{site_url}', '{IMAGE_PATH}');
        $replace_with = array(
            $email_template['setting']['app_name'],
            $get_organizer->event_name,
            $get_organizer->organizer_name,
        ...
        $subject = str_replace($keywords, $replace_with, $email_template['subject']);
        $body_message = str_replace($keywords, $replace_with, $email_template['body']);
        sendMail($to, $subject, '', $body_message);
        $json_array['error'] = 'success';
...
//        /echo 'test';
    ...
    function notification_passcode_validation() {
        $attendee_id = $this->session->userdata('client_attendee_id');
        $event_id = $this->input->post('event_id');
        $passcode_validation = $this->common_transaction_model->common_notification_passcode_validation($event_id, $attendee_id);
        if ($passcode_validation) {
            $json_array['error'] = 'success';
        } else {
            $json_array['error'] = 'error';
        }
