...
class email_template_model extends CI_Model {
    ...
    public $order_by = 'ASC';
    public $order_name = 'email_template.name';
    public $object;
...
//  echo '<pre>';print_r($arrResult);exit;
        ...
        if (isset($id)) {
            $arrData['action'] = base_url() . 'manage/email_template/edit/' . $id;
        } else {
            $arrData['action'] = base_url() . 'manage/email_template/add';
        }
    ...
    function save($data, $id = NULL) {
        ...
        if (is_null($id)) {
            ...
            $result = $this->db->insert('email_template', $arrData);
            $id = $this->db->insert_id();
        } else {
            $arrData['modified_by'] = getCreatedUserId();
            $arrData['modified_date'] = date("Y-m-d H:i:s");
            $this->db->where('id', $id);
            $result = $this->db->update('email_template', $arrData);
        }
        if ($result) {
            return $id;
        } else {
     ...
     * @return  void
     */
    function getAll($id = NULL, $row = FALSE, $search = NULL, $fields = array(), $where = 'email_template.id', $type = 'LIKE') {
        ...
        $this->db->group_by('email_template.id');
