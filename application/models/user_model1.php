...
class user_model extends CI_Model {
    ...
    function save($data, $id = NULL) {
        ...
        if (isset($arrData['email']) && is_null($id)) {
            ...
            $id = $this->check_user($arrData['email']);
        }
        if (is_null($id)) {
            if (isset($arrData['username'])) {
                $id = $this->check_user($arrData['username']);
            }
        }
...
//        echo '<pre>';print_r($objExh);exit;
    ...
    function forgot_password($data = array()) {
        if (empty($data))
            return false;
        $arrResult = array();
        $id = $this->check_user($data['username']);
        if ($id) {
            ...
            if (sendMail($data['username'], $subject, $message)) {
                $arrResult['password'] = ($password);
                $this->save($arrResult, $id);
                return true;
            } else {
                return false;
            }
