...
//            $this->db->where('S_A.attendee_id',$this->attendee_id);
    ...
    function update_attendee($attendee_id, $event_id) {
    ...
    function getActiveAttendee($event_id, $attendee_id = NULL) {
        if ($this->search) {
            $search = $this->search;
            $this->db->where("(A_T.name LIKE '%$search%'|| U_T.first_name LIKE '%$search%' || U_T.last_name LIKE '%$search%')");
        }
        $query = $this->db
