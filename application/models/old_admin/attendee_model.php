...
//                $subject = $objAttend->event_name . ' - Networking App by Procialize - log in credentials'; //'Welcome to Procialize';
                /* $message = "
                  Welcome to Procialize - the platform where you can Professionally Socialize !

                  Here are details of the Event for which you have subscribed:

                  Event Name: {$objAttend->event_name}

                  Event Duration: " . date('Y-m-d', strtotime($objAttend->event_start)) . " To " . date('Y-m-d', strtotime($objAttend->event_end)) . "

                  Event Venue Address: {$objAttend->event_location},{$objAttend->event_city},{$objAttend->event_country}

                  Contact First Name: {$objAttend->first_name}

                  Passcode for the Event: {$objAttend->passcode}

                  Feel free to reachout to us for any queries !

                  Team Procialize"; */
//                echo $message;
//                $message = passcode_to_attendee_speaker();
//                $message = str_replace('{event_name}', $objAttend->event_name, $message);
//                $message = str_replace('{duration}', date('Y-m-d', strtotime($objAttend->event_start)) . " To " . date('Y-m-d', strtotime($objAttend->event_end)), $message);
//                $message = str_replace('{address}', $objAttend->event_location . ',' . $objAttend->event_city . ',' . $objAttend->event_country, $message);
//                $message = str_replace('{first_name}', $objAttend->first_name, $message);
//                $message = str_replace('{passcode}', $objAttend->passcode, $message);
//                $message = str_replace('{password}', '12345', $message);
                //MAIL TEMLATE***
                $email_template = get_email_template('passcode_to_attendee_speaker');
                $keywords = array('{app_name}', '{event_name}', '{duration}', '{address}', '{first_name}', '{passcode}', '{password}', '{app_contact_email}', '{site_url}', '{IMAGE_PATH}', '{logo_image}');
                $replace_with = array(
                ...
                if (isset($objAttend->subscribe_email)) {
                    ...
                    $count++;
                    //$this->db->where('event_id', $objAttend->event_id);
                    //$this->db->where('id', $id);
